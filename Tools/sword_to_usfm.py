...
"""Convert a CrossWire SWORD Bible module into a USFM zip that build_bibles.py reads.
    ...
    pip install pysword
    python3 Tools/sword_to_usfm.py JapKougo    # Data/source/crosswire/JapKougo.zip -> Data/source/jpn-kougo_usfm.zip
...
is marked as a draft, and its Korean is the archaic 1910 translation — so the standard public
domain texts come from CrossWire instead (docs/localization.md).

What the modules carry, and what becomes of it:
- `<w gloss="かみ">神</w>` (JapKougo): furigana. The base text is kept, the reading dropped —
...
MODULES = {
    "JapKougo": {
        "out": "jpn-kougo_usfm.zip",
        "names": [
            "創世記", "出エジプト記", "レビ記", "民数記", "申命記", "ヨシュア記", "士師記", "ルツ記",
            "サムエル記上", "サムエル記下", "列王紀上", "列王紀下", "歴代志上", "歴代志下", "エズラ記",
            "ネヘミヤ記", "エステル記", "ヨブ記", "詩篇", "箴言", "伝道の書", "雅歌", "イザヤ書",
            "エレミヤ書", "哀歌", "エゼキエル書", "ダニエル書", "ホセア書", "ヨエル書", "アモス書",
            "オバデヤ書", "ヨナ書", "ミカ書", "ナホム書", "ハバクク書", "ゼパニヤ書", "ハガイ書",
            "ゼカリヤ書", "マラキ書", "マタイによる福音書", "マルコによる福音書", "ルカによる福音書",
            "ヨハネによる福音書", "使徒行伝", "ローマ人への手紙", "コリント人への第一の手紙",
            "コリント人への第二の手紙", "ガラテヤ人への手紙", "エペソ人への手紙", "ピリピ人への手紙",
            "コロサイ人への手紙", "テサロニケ人への第一の手紙", "テサロニケ人への第二の手紙",
            "テモテへの第一の手紙", "テモテへの第二の手紙", "テトスへの手紙", "ピレモンへの手紙",
            "ヘブル人への手紙", "ヤコブの手紙", "ペテロの第一の手紙", "ペテロの第二の手紙",
            "ヨハネの第一の手紙", "ヨハネの第二の手紙", "ヨハネの第三の手紙", "ユダの手紙",
            "ヨハネの黙示録",
        ],
    },
    "KorRV": {
            ...
            for index, (code, book, name) in enumerate(zip(BOOKS, canon, spec["names"]), start=1):
                ...
                for chapter in range(1, book.num_chapters + 1):
                    ...
                    for verse in range(1, book.chapter_lengths[chapter - 1] + 1):
                        ...
                        lines.append(f"\\p\n\\v {verse} {text}")
                        verses += 1
                out.writestr(f"{index:02d}-{code}.usfm", "\n".join(lines) + "\n")
