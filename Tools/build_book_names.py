#!/usr/bin/env python3
"""Generate ScriptureAloneCore/Sources/ScriptureAloneCore/BookNamesTable.swift.
...
language's standard short forms. Generated into Swift source rather than shipped as a resource so
the widgets and the watch — separate bundles — have it too.
...
def main():
    ...
    for lang, tid in LANGUAGES:
        ...
        rows = []
        for i in range(66):
            aliases = [abbreviations[i]] + extras.get(i + 1, [])
            rows.append(f"        ({swift_string(names[i])}, {swift_string(abbreviations[i])}, "
