...
final class StudyModel {
    ...
    ///
    /// It ships inside the app, so it is always present; opening is deferred only because 43.6 MB
    /// is not worth paging in for a reader who never opens Commentary.
    var store: StudyStore? {
        if let loadedStore { return loadedStore }
        guard !loadFailed else { return nil }
        guard let url = StudyPack.commentary.url else { return nil }
