...
public struct ESVClient: Sendable {
    ...
    public func chapter(_ chapter: ChapterRef, session: URLSession = .shared) async throws -> [VerseText] {
        ...
        let verses = Self.parse(body, in: chapter)
    ...
    /// Splits Crossway's plain text into verses.
    ///
    /// The API marks verses as `[3]`, and the first verse of a chapter is marked with the chapter
